...
class LiveActivityManager {
    ...
    func startLiveActivity(data: [String: Any]?) {
        let attributes = LiveActivityWidgetAttributes(name: "LiveActivity")
        if let info = data {
          let state =  LiveActivityWidgetAttributes.ContentState(
            ...
            liveActivity = try? Activity<LiveActivityWidgetAttributes>.request(attributes: attributes,  content: .init(state: state, staleDate: nil), pushType: nil)
